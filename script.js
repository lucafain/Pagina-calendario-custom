...
const icalFeedback = document.getElementById("ical-feedback");
const clearIcalButton = document.getElementById("clear-ical-button");
...
icalForm?.addEventListener("submit", handleIcalSubmit);
clearIcalButton?.addEventListener("click", handleClearIcalSubscription);
  ...
  for (let day = 1; day <= daysInMonth; day += 1) {
    ...
    if (eventsForDay.length) {
      ...
      eventsForDay.forEach((event) => {
        ...
        if (source === "ical") {
          eventChip.classList.add("event-chip--external");
          eventChip.setAttribute("title", "Evento sincronizado desde iCal");
        } else {
          eventChip.setAttribute("title", "Evento personal");
        }
        eventChip.dataset.source = source;

        const timeElement = document.createElement("strong");
        timeElement.textContent = event.displayTime || event.time || "";
        ...
        if (event.location) {
          ...
          eventChip.appendChild(locationElement);
        }
...
function buildNormalizedEvent(event, defaultSource = "manual") {
  ...
  const normalizedTime = isAllDay ? "00:00" : normalizeTimeValue(event.time);
  const displayTime = isAllDay
    ? "Todo el día"
    : event.displayTime || normalizedTime;
  const sortKey = event.sortKey || (isAllDay ? "00:00" : normalizedTime);
  const title = (event.title || "Evento").toString().trim() || "Evento";
...
function normalizeEventList(list, defaultSource = "manual") {
  if (!Array.isArray(list)) return [];
  return list
    .map((event) => buildNormalizedEvent(event, defaultSource))
    .filter(Boolean);
}
...
function sortEvents(list) {
  ...
  return [...list].sort((a, b) => {
    const keyA = (a?.sortKey || a?.time || "").padStart(5, "0");
    const keyB = (b?.sortKey || b?.time || "").padStart(5, "0");
...
function normalizeEventsDictionary(eventsByDate, defaultSource = "manual") {
  const normalized = {};
  ...
  Object.entries(eventsByDate).forEach(([dateKey, list]) => {
    const normalizedList = sortEvents(normalizeEventList(list, defaultSource));
    if (normalizedList.length) {
      normalized[dateKey] = normalizedList;
    }
  });
...
function countEventsInDictionary(eventsByDate) {
  if (!eventsByDate || typeof eventsByDate !== "object") return 0;
  return Object.values(eventsByDate).reduce((acc, list) => {
...
function normalizeIcalSubscription(snapshot) {
  if (!snapshot || typeof snapshot !== "object") {
    return null;
  }

  const events = normalizeEventsDictionary(snapshot.events || {}, "ical");
  const normalized = {
    rawInput: snapshot.rawInput || snapshot.source || "",
    sourceType: snapshot.sourceType || snapshot.type || (snapshot.source ? "url" : "inline"),
    sourceValue: snapshot.sourceValue || snapshot.source || "",
    calendarName: snapshot.calendarName || snapshot.name || "",
    lastUpdated: snapshot.lastUpdated || new Date().toISOString(),
    events,
    totalEvents:
      snapshot.totalEvents != null
        ? snapshot.totalEvents
        : countEventsInDictionary(events),
  };

  return normalized;
}

function cloneIcalSubscription(subscription) {
  if (!subscription) return null;
  return {
    rawInput: subscription.rawInput || "",
    sourceType: subscription.sourceType || "inline",
    ...
    totalEvents:
      subscription.totalEvents != null
        ? subscription.totalEvents
        : countEventsInDictionary(subscription.events),
    events: deepCloneEvents(subscription.events || {}),
  };
}
...
function getEventsForDate(dateKey) {
  const manualEvents = normalizeEventList(state.events[dateKey], "manual");
  const externalEvents = state.icalSubscription?.events?.[dateKey] || [];
  const normalizedExternal = normalizeEventList(externalEvents, "ical");
  const combined = [...manualEvents, ...normalizedExternal];
...
function handleEventSubmit(event) {
  ...
  const newEvent = buildNormalizedEvent(
    {
      title,
      time,
      displayTime: time,
  ...
  state.events[state.activeDateKey].push(newEvent);
  state.events[state.activeDateKey] = sortEvents(state.events[state.activeDateKey]);
...
function hydrateStateFromSnapshot(snapshot) {
  state.events = normalizeEventsDictionary(snapshot?.events || {}, "manual");
  state.selectedTheme = cloneTheme(snapshot?.selectedTheme) || DEFAULT_THEME;
  state.themeLocked = Boolean(snapshot?.themeLocked);
  state.icalSubscription = normalizeIcalSubscription(snapshot?.icalSubscription);
...
function setIcalFeedback(message, status = "info") {
  if (!icalFeedback) return;
  icalFeedback.textContent = message || "";
  if (!status) {
    icalFeedback.removeAttribute("data-status");
  } else {
    icalFeedback.setAttribute("data-status", status);
  }
}

function updateIcalStatus() {
  if (!icalStatus) return;

  if (!state.icalSubscription) {
    icalStatus.textContent = "Todavía no conectaste un calendario iCal.";
  } else {
    const name = state.icalSubscription.calendarName?.trim() || "Calendario iCal";
    const total =
      state.icalSubscription.totalEvents != null
        ? state.icalSubscription.totalEvents
        : countEventsInDictionary(state.icalSubscription.events);
    const updatedLabel = formatStatusTimestamp(state.icalSubscription.lastUpdated);
    let message = `${name} sincronizado`;
    if (total > 0) {
      message += ` · ${total} evento${total === 1 ? "" : "s"}`;
    }
    if (updatedLabel) {
      message += ` · Actualizado ${updatedLabel}`;
    }
    icalStatus.textContent = message;
  }

  if (clearIcalButton) {
    clearIcalButton.disabled = !state.icalSubscription;
  }
}

function updateIcalUIFromState(forceReset = false) {
  if (icalInput) {
    if (state.icalSubscription?.rawInput) {
      icalInput.value = state.icalSubscription.rawInput;
    } else if (forceReset) {
      icalInput.value = "";
    }
  }
...
async function handleIcalSubmit(event) {
  event.preventDefault();
  if (!icalInput) return;

  if (!state.profile) {
    setIcalFeedback("Guardá tu perfil antes de conectar un calendario iCal.", "error");
    return;
  }

  const raw = icalInput.value.trim();
  if (!raw) {
    setIcalFeedback("Pegá un enlace o código de inserción iCal para sincronizar.", "error");
    return;
  }

  setIcalFeedback("Importando eventos de iCal…", "loading");

  try {
    const payload = await resolveIcalPayload(raw);
    if (!payload?.ics) {
      throw buildIcalError(
        "INVALID_ICS",
        "No pudimos leer ningún evento en el código que pegaste."
      );
    }

    const parsed = parseIcsCalendar(payload.ics);
    if (!parsed || countEventsInDictionary(parsed.events) === 0) {
      throw buildIcalError(
        "EMPTY_ICS",
        "No encontramos eventos en ese calendario. Verificá que tenga reuniones o recordatorios."
      );
    }

    state.icalSubscription = normalizeIcalSubscription({
      rawInput: raw,
      ...
      totalEvents: countEventsInDictionary(parsed.events),
    });

    updateIcalUIFromState();
    renderCalendar();
    persistUserState();

    const total = state.icalSubscription.totalEvents || 0;
    const calendarName = state.icalSubscription.calendarName || "tu calendario iCal";
    setIcalFeedback(
      `Sincronizamos ${total} evento${total === 1 ? "" : "s"} desde ${calendarName}.`,
      "success"
    );
  } catch (error) {
    console.error("No se pudo importar el calendario iCal", error);
    if (error?.name === "ICAL_FETCH_ERROR") {
      setIcalFeedback(
        "No pudimos descargar el enlace. Verificá que sea público o copiá el contenido del archivo .ics.",
        "error"
      );
    } else if (error?.name === "INVALID_ICS" || error?.name === "EMPTY_ICS") {
      setIcalFeedback(error.message, "error");
    } else {
      setIcalFeedback(
        "Ocurrió un problema al sincronizar. Probá nuevamente o pegá el archivo .ics completo.",
        "error"
      );
    }
  }
...
function handleClearIcalSubscription(event) {
  event.preventDefault();
  if (!state.icalSubscription) {
    setIcalFeedback("No hay un calendario iCal conectado en este momento.");
    updateIcalStatus();
    return;
  }

  state.icalSubscription = null;
  updateIcalUIFromState(true);
  renderCalendar();
  persistUserState();
  setIcalFeedback("Se quitó la sincronización con iCal.", "success");
}

async function resolveIcalPayload(raw) {
  ...
  if (/BEGIN:VCALENDAR/i.test(trimmed)) {
    return { type: "inline", source: "inline", ics: trimmed };
  }

  const url = extractIcalUrl(trimmed);
  if (!url) {
    return null;
  }
  ...
  try {
    ...
    if (!response.ok) {
      throw buildIcalError("ICAL_FETCH_ERROR", `Respuesta ${response.status}`);
    }
    const text = await response.text();
    if (!/BEGIN:VCALENDAR/i.test(text)) {
      throw buildIcalError("INVALID_ICS", "El enlace no contiene un archivo .ics válido.");
    }
    return { type: "url", source: url, ics: text };
  } catch (error) {
    if (error.name !== "ICAL_FETCH_ERROR") {
      throw buildIcalError("ICAL_FETCH_ERROR", error.message, error);
    }
    throw error;
  }
}

function extractIcalUrl(input) {
  const srcMatch = input.match(/src\s*=\s*"([^"']+)"/i);
  if (srcMatch?.[1]) {
    return srcMatch[1];
  }
  const hrefMatch = input.match(/https?:\/\/[^\s"'>]+/i);
  if (hrefMatch?.[0]) {
    return hrefMatch[0];
  }
  return null;
...
function buildIcalError(name, message, cause) {
  ...
  lines.forEach((rawLine) => {
    ...
    if (upper === "END:VEVENT") {
      if (currentEvent?.dateKey) {
        ...
        if (normalized) {
          if (!eventsByDate[currentEvent.dateKey]) {
            eventsByDate[currentEvent.dateKey] = [];
          }
          eventsByDate[currentEvent.dateKey].push(normalized);
        }
      }
      currentEvent = null;
      return;
    ...
    const propertyKey = property.toUpperCase();

    if (!currentEvent) {
      if (
        !calendarName &&
        (propertyKey === "X-WR-CALNAME" || propertyKey === "NAME" || propertyKey === "CALNAME")
      ) {
        calendarName = decodeIcsText(value);
      }
      return;
    }

    if (propertyKey.startsWith("SUMMARY")) {
      currentEvent.title = decodeIcsText(value);
      return;
    }

    if (propertyKey.startsWith("DTSTART")) {
      const timing = parseIcsDate(property, value);
      if (timing) {
        currentEvent.dateKey = timing.dateKey;
        currentEvent.time = timing.time;
        currentEvent.displayTime = timing.displayTime;
        currentEvent.sortKey = timing.sortKey;
        currentEvent.isAllDay = timing.isAllDay;
      }
      return;
    }

    if (propertyKey.startsWith("UID")) {
      currentEvent.uid = value.trim();
      return;
    }

    if (propertyKey.startsWith("LOCATION")) {
      currentEvent.location = decodeIcsText(value);
    }
  });

  Object.keys(eventsByDate).forEach((dateKey) => {
    eventsByDate[dateKey] = sortEvents(eventsByDate[dateKey]);
  });
...
function parseIcsDate(property, rawValue) {
  if (!rawValue) return null;
  const trimmedValue = rawValue.trim();
  if (!trimmedValue) return null;

  const upperProperty = property.toUpperCase();
  const isDateOnly = upperProperty.includes("VALUE=DATE") || !trimmedValue.includes("T");
  const isUtc = trimmedValue.endsWith("Z");
  const sanitized = trimmedValue.replace(/Z$/, "");
  ...
  let date;
  if (isDateOnly || timePartRaw.length < 3) {
    date = new Date(year, monthIndex, day);
  } else {
    const hours = Number(timePartRaw.slice(0, 2));
    const minutes = Number(timePartRaw.slice(2, 4));
    if (Number.isNaN(hours) || Number.isNaN(minutes)) {
      date = new Date(year, monthIndex, day);
    } else if (isUtc) {
      date = new Date(Date.UTC(year, monthIndex, day, hours, minutes));
    } else {
...
function persistUserState() {
  ...
  state.events = normalizeEventsDictionary(state.events, "manual");
  state.icalSubscription = normalizeIcalSubscription(state.icalSubscription);
  ...
  users[key] = {
    ...
    icalSubscription: cloneIcalSubscription(state.icalSubscription),
  };
...
function generateSyncCode() {
  ...
  const manualEvents = normalizeEventsDictionary(state.events, "manual");
  const icalSubscription = normalizeIcalSubscription(state.icalSubscription);
  ...
  const snapshot = {
    ...
    name: state.profile.name,
    device: state.profile.device,
    events: deepCloneEvents(manualEvents),
    selectedTheme: cloneTheme(state.selectedTheme),
    themeLocked: state.themeLocked,
  };

  if (icalSubscription) {
    snapshot.icalSubscription = cloneIcalSubscription(icalSubscription);
  }
